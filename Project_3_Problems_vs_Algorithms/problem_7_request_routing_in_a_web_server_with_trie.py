...
The purpose of an HTTP Router is to take a URL path like "/", "/about", or "/blog/2019-01-15/my-awesome-blog-post" and
figure out what content to return. In a dynamic web server, the content will often come from a block of code called a
handler.
...
things would be on the parts of the path that are separated by slashes ("/"). A Trie with a single path entry
...
class RouteTrie:
    ...
    def insert(self, path_parts, handler):
        ...
        for part in path_parts[:-1]:
            current.insert(part, None)
            current = current.children[part]
        # Finally, insert the leaf node with the path handler.
        current.insert(path_parts[-1], handler)
...
class RouteTrieNode:
    ...
    def __init__(self, handler=None):
        # If handler is None, it means the Node is an inner Node, if not it means it's a leaf node.
        self.children = {}
...
"""
Next we need to implement the actual Router. The router will initialize itself with a RouteTrie for holding routes and
associated handlers. It should also support adding a handler by path and looking up a handler by path. All of these
operations will be delegated to the RouteTrie.
...
class Router:
    ...
    def add_handler(self, path, handler):
        ...
        self.route_trie.insert(self._split_path(path), handler)
    ...
    def lookup(self, path):
        ...
        handler = self.route_trie.find(self._split_path(path))
    ...
    def _split_path(self, path):
        ...
        # Strip out '/' in the ends, so empty parts at the beginning and at the end are not handled.
        from_ = 1 if path[0] == '/' else 0
        to = len(path) - 1 if path[-1] == '/' else len(path)
        return path[from_:to].split('/')
...
router.add_handler("/home/users", "users handler")
router.add_handler("/home/users/me", "single_user handler")
router.add_handler("/hello", "hello handler")
router.add_handler("/hello/world", "hello_world handler")
...
print((router.lookup("/home/users")))  # should print 'users handler'
print((router.lookup("/home/uses")))  # should print 'not found handler'

print((router.lookup("/home/users/me/")))  # should print 'single_user handler'
print((router.lookup("/home/users/myself/")))  # should print 'not found handler'

print((router.lookup("/hello")))  # should print 'hello handler'
print((router.lookup("/hello/world")))  # should print 'hello_world handler'
print((router.lookup("/hello/world/earth")))  # should print 'not found handler'
